...
const experience = [
  {
    role: 'Cyber Security Engineer', company: 'Cloud4C Services', location: 'Hyderabad', dates: 'Jul 2024 — Present',
    points: [
  ...
  {
    role: 'Cyber Security Intern', company: 'Cloud4C Services', location: 'Hyderabad', dates: 'Feb 2024 — May 2024',
    points: [
...
function Matrix() {
  useEffect(() => {
    ...
    resize();
    const font = 13;
    let columns = Math.ceil(canvas.width / font);
    let drops = Array(columns).fill(1).map(() => Math.random() * -50);
    const draw = () => {
      ctx.fillStyle = 'rgba(5,8,11,.09)'; ctx.fillRect(0, 0, canvas.width, canvas.height);
      ctx.fillStyle = 'rgba(0,245,160,.28)'; ctx.font = `${font}px monospace`;
    ...
    draw();
    window.addEventListener('resize', () => { resize(); columns = Math.ceil(canvas.width / font); drops = Array(columns).fill(1).map(() => Math.random() * -50); });
...
function App() {
  ...
  const target = 'Aditya Kumar — Cyber Security Engineer';

  useEffect(() => {
    let i = 0; const timer = setInterval(() => { setTyped(target.slice(0, i)); i += 1; if (i > target.length) clearInterval(timer); }, 45);
  ...
  return <div className={dark ? 'app dark' : 'app light'}>
    <header className="nav">
      <a className="brand" href="#top"><span className="prompt">./</span>aditya<span className="accent">@sec</span></a>
      <nav>{['about','skills','certifications','experience','contact'].map(x => <a key={x} href={`#${x}`}>{x}</a>)}</nav>
    ...
    <main id="top">
      <section className="hero">
        ...
        <div className="hero-content">
          <div className="terminal-line"><span className="accent">&gt;</span> whoami</div>
          <h1>{typed}<span className="cursor">_</span></h1>
          <p>Enterprise security platform engineering, SIEM administration, vulnerability management and endpoint security.</p>
          <div className="hero-meta"><span>HYDERABAD, INDIA</span><span>•</span><span>2+ YEARS</span></div>
          <div className="actions"><a className="btn primary" href="#contact">Initialize Contact</a><a className="btn" href="/adityaportfolio/resume.pdf" download>Download CV</a><a className="btn" href="https://github.com/AdityaK2608" target="_blank" rel="noreferrer">GitHub ↗</a></div>
        </div>
        <div className="scroll-hint">scroll_to_explore ↓</div>
      </section>

      <section id="about" className="section reveal"><div className="section-label">01 / about</div><div className="about-grid"><div><h2>Security, systems,<br/><span className="accent">and visibility.</span></h2></div><p>Cyber Security Engineer with 2+ years of experience in enterprise security platform engineering, specializing in SIEM administration, vulnerability management and endpoint security. Hands-on experience spans IBM QRadar across 500+ MSSP client environments, plus Tenable Nessus, CrowdStrike Falcon, SentinelOne and Trend Micro HBSS. I focus on reliable security operations, detection quality, platform health and measurable remediation outcomes.</p></div></section>
    ...
    <footer><span>© {new Date().getFullYear()} Aditya Kumar</span><span>built with React + Vite</span><span className="accent">status: available</span></footer>
  </div>
}
